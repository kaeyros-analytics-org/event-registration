...
class SalesVisitService {
    ...
    async downloadDataExcelByCode(
        ...
        try {
          ...
          const saleFormData = await SalesVisit.find({
            sale_representative_code : req.params.code,
            createdAt: {
                $gte: startOfDay,
                $lte: endOfDay,
            },
        }).populate('sale_representative_id');
    
          const data = saleFormData.map((item: any) => {
            return {
                "Code du representant": item.sale_representative_code,
                "Nom du representant": item.sale_representative_id.name,
                "Zone du client": item.zone,
                "Nom du client": item.customer_name,
                "Statut du client": item.customer_status,
                "Nom du business": item.business_name,
                "Numéro de téléphone du contact (WhatsApp)": item.contact,
                "Ville": item.city,
                "Quartier": item.address,
                "Catégorie": item.category,
                "Type de business": item.type_of_business,
                "Objectif de visite": item.visit_objective,
                "Decision du client": item.customer_decision,
                "Type de prospection": item.prospecting_type,
                "Prix de lancement proposé": item.suggested_introductory_price,
                "Prix mensuel proposé": item.proposed_monthly_price,
                "Réalisation / résultat atteint par rapport à l'objectif fixé": item.achievement,
                "Commentaire": item.comment,
                "prochaine action": item.visit_carried_out,
                latitude: item.latitude,
                longitude: item.longitude,
            };
